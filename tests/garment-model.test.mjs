...
const hookPoints = (dimensions, roundness = 72) => garmentFaces(dimensions, roundness).filter((face) => face.material === 'hook').flatMap((face) => face.points);
...
test('one circular hook is always attached at the crown centre', () => {
  for (const roundness of [0, 50, 100]) {
    const [x, y] = garmentHookAnchor(roundness);
    assert.equal(x, 0);
    assert.equal(y, garmentTopY(0, roundness));
    assert.ok(garmentFaces(base, roundness).filter((face) => face.material === 'hook').length > 100);
  }
...
test('whole scale scales all axes and geometry stays lightweight', () => {
  ...
  assert.ok(garmentFaces(base, 100).length < 800);
});
